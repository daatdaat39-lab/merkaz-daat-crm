...
function ContactInsights({ insights }) {
  ...
  return (
    <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 4, background: 'var(--bg-secondary, #fafafa)', borderRadius: 8, padding: '10px 12px' }}>
      {row('מחלקות מוכר בהן', departments.length ? departments.join(', ') : 'אף מחלקה')}
      {row('שנת-שיא תרומה', peakDonation ? `${peakDonation.year} · ₪${Math.round(peakDonation.amount).toLocaleString('he-IL')}` : '—')}
      {row('סה"כ תרומות', totalDonations.count > 0 ? `${totalDonations.count} תרומות · ₪${Math.round(totalDonations.total).toLocaleString('he-IL')}` : 'אין')}
      ...
      {(hasActiveCommitment || coursesCount > 0 || seminarsCount > 0) && (
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 2 }}>
          {hasActiveCommitment && <span style={pillStyle()}>הוראת קבע פעילה</span>}
...
function pillStyle() {
  return { fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 999, background: 'var(--accent-soft, #e4ede8)', color: 'var(--accent, #1f4d3d)' };
}
