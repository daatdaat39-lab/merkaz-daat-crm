...
export default function PipelineBoard({ contacts, moveStageAction, stages }) {
  ...
  function moveContact(departmentRowId, newStage) {
    setLocalContacts((prev) => prev.map((c) => (c.departmentRowId === departmentRowId ? { ...c, stage: newStage } : c)));
    startTransition(async () => {
      await moveStageAction(departmentRowId, newStage);
    });
  }
  ...
  function handleDrop(e, stage) {
    ...
    moveContact(departmentRowId, stage);
  }
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <span style={{ fontSize: 11.5, color: 'var(--text-muted)' }}>לסיום מעורבות עם ליד — הסרה ממחלקה מתוך הכרטיס שלו</span>
            ...
            {filtered.map((c) => (
              <tr key={c.departmentRowId} style={{ borderBottom: '1px solid #f2f2f2' }}>
                ...
                <td style={{ padding: '10px 16px', fontSize: 13 }}>
                  <select
                    value={c.stage}
                    onChange={(e) => moveContact(c.departmentRowId, e.target.value)}
                    style={{
