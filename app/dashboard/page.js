        ...
        <thead>
          <tr style={{ background: '#EFE6CC' }}>
            {['שם','טלפון','מייל','תחום','תגיות'].map(h => (
              <th key={h} style={{ textAlign: 'right', fontSize: 11.5, color: '#6B6151', padding: '10px 12px' }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          ...
          {(!contacts || contacts.length === 0) && (
            <tr><td colSpan={5} style={{ padding: '10px 12px', fontSize: 13, color: '#6B6151' }}>אין אנשי קשר</td></tr>
          )}
        </tbody>
      </table>
    </div>
