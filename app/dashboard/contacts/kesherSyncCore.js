// ליבת סנכרון קשר - מודול רגיל (לא 'use server') כדי שאפשר יהיה לקרוא
// לו גם מפעולת-שרת ידנית (kesherSyncActions.js, עם session אמיתי) וגם
// מ-Route Handler של cron (app/api/cron/sync-kesher, עם service-role
// client) בלי מגבלות ה-serialization של Server Actions (שדורשות
// שהפרמטרים יהיו ניתנים לסריאליזציה - supabase client שמועבר כאן לא
// עומד בזה). ר' lib/kesher/client.js - הקובץ הזה לעולם לא קורא לשום
// פונקציה מחייבת של קשר, גם לא בעקיפין.
...
// הליבה עצמה - נקראת גם מהפעולה הידנית (kesherSyncActions.js, עם
// session אמיתי) וגם מה-cron האוטומטי (app/api/cron/sync-kesher, עם
// service-role client ובלי משתמש מחובר - user={id:null}). כל שימוש
