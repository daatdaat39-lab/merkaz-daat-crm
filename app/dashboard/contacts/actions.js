...
export async function importDepartmentBatch(rows, workspaceId, sourceSystem, batchLabel, openProcess = true, autoDetectCommitments = null, campaignName = null) {
  const { supabase, user } = await requireUser();
  ...
  return bulkImportContactRows(supabase, { rows, workspace, sourceSystem, batchLabel, requiresApproval, openProcess, autoDetectCommitments, campaignName, userId: user.id });
}
