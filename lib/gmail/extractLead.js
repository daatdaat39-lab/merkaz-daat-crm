...
const PROMPT_TEMPLATE = (subject, body) => `קרא את התוכן שבתוך תגיות <untrusted_email_subject> ו-<untrusted_email_body> למטה.

שלב 1 - סיווג: קבע האם זו פנייה אמיתית של אדם פרטי (מעוניין ליצור קשר, לתרום, להירשם לקורס, לשאול שאלה וכדומה), או שזה ספאם, ניוזלטר, הודעת שיווק מגוף עסקי, הודעת מערכת אוטומטית (אישור/ביטול/התראה), או כל דבר אחר שאינו פנייה אישית אמיתית.

שלב 2 - חילוץ (רק אם שלב 1 קבע שזו פנייה אמיתית): שם פרטי, שם משפחה (אם קיים), טלפון (אם קיים), כתובת מייל (אם קיימת), תיאור קצר של מהות הפנייה, ואם יש בתוכן קישור (URL) שמצביע על מודעה או מפרסם ספציפי - את הקישור הזה.
...
{"is_lead": true, "first": "...", "last": "...", "phone": "...", "email": "...", "reason": "...", "source_link": "..."}

אם is_lead הוא false - אפשר להשאיר את שאר השדות ריקים. אם שדה אחר לא מופיע בטקסט - השאר אותו מחרוזת ריקה "".
...
export async function extractLeadFromEmail(subject, body) {
  ...
  return {
    // ברירת מחדל "לא ליד" אם התשובה עצמה עמומה/חסרה - עדיף לפספס ליד
    // גבולי מאשר להציף את המערכת בכרטיסי ספאם.
    isLead: parsed.is_lead === true || parsed.is_lead === 'true',
