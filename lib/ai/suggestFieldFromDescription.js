...
לשדה מחושב, מותר לך להשתמש רק באופרטורים + - * / וסוגריים, בשמות השדות הקיימים בדיוק כפי שמופיעים למעלה (או המילה today לתאריך היום), ובפונקציות הבאות בלבד: months_between(a,b), days_between(a,b), years_between(a,b), percent(a,b). דוגמה: percent(donation_paid, donation_total).

החזר אך ורק JSON תקין (בלי טקסט נוסף לפני/אחרי), באחד משני הפורמטים הבאים בדיוק:

לשאלת הבהרה: {"needsClarification": true, "question": "..."}

להצעה סופית: {"needsClarification": false, "fieldKey": "מפתח_קצר_באנגלית", "label": "תווית בעברית", "type": "computed", "expression": "...", "unit": "יחידה אופציונלית כמו %/חודשים, או null", "explanation": "משפט קצר בעברית שמסביר מה השדה מחשב"}

או עבור שדה רגיל (לא מחושב): {"needsClarification": false, "fieldKey": "...", "label": "...", "type": "text"|"number"|"date"|"select", "options": ["אופציה1","אופציה2"] (רק אם type=select, אחרת מערך ריק), "explanation": "..."}`;
}
