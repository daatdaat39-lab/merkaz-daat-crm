...
מותר גם להשתמש באופרטורי השוואה (> < >= <= == !=) ובפונקציה if(תנאי, ערך-אם-כן, ערך-אם-לא) - זה מה שמאפשר שדות "סטטוס" חכמים. דוגמה: if(days_since_last_transaction() > 365, "לא פעיל", "פעיל") - "אם עברו יותר מ-365 ימים מהתנועה האחרונה, הצג 'לא פעיל', אחרת 'פעיל'".

מותר גם להשתמש בפונקציות מצטברות הבאות - **בלי שום ארגומנטים בכלל** (לא שמות שדות!), הן קוראות אוטומטית את כל היסטוריית התנועות הכספיות (תרומות/תשלומים) של אותו איש קשר במחלקה:
- total_transactions() - סה"כ סכום כל התנועות אי-פעם
- transaction_count() - כמה תנועות יש
- avg_transaction() - ממוצע לתנועה
- days_since_last_transaction() - כמה ימים עברו מהתנועה האחרונה
- first_transaction_date() / last_transaction_date() - תאריך התנועה הראשונה/אחרונה (אפשר לשלב עם months_between/years_between, למשל months_between(first_transaction_date(), today))
- total_transactions_this_year() - סכום תנועות בשנה הקלנדרית הנוכחית
השתמש בפונקציות האלה כשהבקשה נוגעת להיסטוריית תרומות/תשלומים כללית (ממוצע, סה"כ, מתי לאחרונה, סטטוס פעיל/לא-פעיל) - לא כשמדובר בשדה ספציפי בודד שכבר קיים ברשימת השדות.
...
הצעה סופית לשדה מחושב (סטטוס מבוסס תנאי + היסטוריית תנועות): {"needsClarification": false, "fieldKey": "donor_status", "label": "סטטוס תרומה", "type": "computed", "expression": "if(days_since_last_transaction() > 365, \"לא פעיל\", \"פעיל\")", "unit": null, "explanation": "פעיל אם תרם ב-365 הימים האחרונים"}
