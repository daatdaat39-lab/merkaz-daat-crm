...
export async function addCategoryViewTabs(accessToken, spreadsheetId, sheetTitle, categoryOptions = [], existingTitles = new Set()) {
  ...
  const data = tabs.map((t) => ({
    ...
    values: [
      ...
      // טווח פתוח (בלי מספר-שורה-סופי) - קמפיינים גדולים (אלפי אנשי קשר)
      // חורגים בקלות מ-2000 שורות, וגבול קבוע היה מחסיר אותם מהתצוגה.
      [`=QUERY('${sheetTitle}'!A2:Q, "select * where ${t.column} = '${escapeQuery(t.value)}'", 0)`],
    ],
  }));
