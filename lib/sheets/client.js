...
export async function addCategoryViewTabs(accessToken, spreadsheetId, sheetTitle, categoryOptions = []) {
  ...
  // אותה כוונה בדיוק - "לא שייך לקמפיין הזה").
  const tabs = [
  ...
  const data = tabs.map((t) => ({
    ...
    values: [
      ...
      [`=QUERY('${sheetTitle}'!A1:Q2000, "select * where ${t.column} = '${escapeQuery(t.value)}'", 1)`],
    ],
  }));
