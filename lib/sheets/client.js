...
export async function formatCampaignSheet(accessToken, spreadsheetId, sheetId, { categoryOptions = [], statusOptions = [], decisionOptions = [], columnCount = 17, dataRowCount = 10000 } = {}) {
...
export async function addCategoryViewTabs(accessToken, spreadsheetId, sheetTitle, categoryOptions = [], existingTitles = new Set()) {
  ...
  const data = tabs.map((t) => ({
    ...
    values: [
      ...
      [`=QUERY('${sheetTitle}'!A1:Q, "select * where ${t.column} = '${escapeQuery(t.value)}'", 1)`],
    ],
  }));
