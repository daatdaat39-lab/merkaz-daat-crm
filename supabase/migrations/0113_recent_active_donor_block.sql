-- חסימה אוטומטית מהתור: מי שיש לו הוראת-קבע פעילה (commitments.status=
-- 'active') וגם תרם בפועל (donation_transactions) ב-40 הימים האחרונים -
-- לא צריך שיתקשרו אליו שוב על אותה בקשה. allow_recent_donor_call הוא
-- override פר-שורה (לא פר-איש-קשר-גלובלי) - מנהל יכול "לפתוח" מקרה
-- ספציפי אם בכל זאת רוצה שיתקשרו אליו.
alter table public.campaign_contacts
  add column if not exists allow_recent_donor_call boolean not null default false;

-- לשימוש בטבלת-הניהול (CampaignDetailClient) - כל מזהי-אנשי-הקשר
-- בקמפיין הזה שחוסמים אוטומטית, בשאילתה אחת יעילה (לא .in() עם אלפי
-- מזהים - ר' הבאג שתוקן מוקדם יותר עם bulkUpdateCampaignContactsField).
create or replace function public.get_recent_active_donor_contact_ids(p_campaign_id uuid, p_days int default 40)
returns table(contact_id uuid)
language sql stable as $$
  select distinct cc.contact_id
  from public.campaign_contacts cc
  where cc.campaign_id = p_campaign_id
    and exists (select 1 from public.commitments cm where cm.contact_id = cc.contact_id and cm.status = 'active')
    and exists (
      select 1 from public.donation_transactions dt
      where dt.contact_id = cc.contact_id and dt.transaction_date >= (current_date - (p_days || ' days')::interval)
    );
$$;

-- אותה חתימה בדיוק כמו 0102 - create or replace אמיתי (לא overload חדש).
...
-- אותה חתימה בדיוק כמו 0110 - create or replace אמיתי (לא overload חדש).
