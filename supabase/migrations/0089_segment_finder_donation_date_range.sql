-- מוסיף סינון "תרם בין תאריך X לתאריך Y" (לפי donation_transactions.
-- transaction_date, כל תרומה - לא רק שנת-השיא) לבניית-קבוצה. זה קריטריון
-- עצמאי ומצטרף ב-AND לכל שאר הפילטרים (כולל p_exclude_donated_within_days)
-- בדיוק כמו כל שאר השורות ב-WHERE - כלומר "הוצא מי שתרם לאחרונה" ממשיך
-- להחריג גם כשיש טווח-תאריכים פעיל, לא רק בתוך הטווח שנבחר.
drop function if exists public.find_campaign_segment_candidates(
  text, uuid, text[], numeric, numeric, boolean, boolean, boolean, int, uuid, text, text, int, int
);
