-- תור-אישי לכל טלפן: לא חסימה (claim_scope נשאר 'anyone', לא נוגעים בו) -
-- רק עדיפות. שורה עם assigned_to=הנציג-המבקש מקבלת שכבת-עדיפות חדשה,
-- מיד אחרי בדיקות-הבטיחות הקיימות (תפוסה-בפועל/דילוג-אישי) ולפני תור-
-- החזרה/הכללי - כך שכל עוד יש לנציג שורות-אישיות זמינות, הן תמיד
-- קודמות (כולל התחשבות בסדר הפנימי הרגיל - שיחות-חזרה-אישיות-שלו
-- קודמות לשורות-אישיות-אחרות, בדיוק כמו שההיררכיה הקיימת כבר עובדת
-- בתוך המאגר הכללי). ברגע שאין לנציג יותר שורות-אישיות זמינות, הוא
-- "נופל" אוטומטית לתור הכללי - באותה שאילתה, בלי מתג נפרד. אף נציג לא
-- חסום מלתפוס שורה-לא-שלו (רק לא מקבל עדיפות עליה) - מנהל עדיין יכול
-- להקצות/לשנות דרך "שליחה לנציג" הקיים ב-BulkAssignBar.
-- מוסיפים גם assigned_to ל-RETURNS TABLE כדי שהלקוח ידע אם הכרטיס
-- שנתפס אישי-לנציג או מהמאגר הכללי (לתגית "מהרשימה האישית שלך").
drop function if exists public.claim_next_campaign_contact(
  uuid, uuid, text, text[], int, int, uuid
);
