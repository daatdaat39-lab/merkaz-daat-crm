-- מאפשר לבחור כמה שלבים בבת-אחת בסינון-לפי-שלב (במקום שלב בודד) - "או"
-- בין השלבים שנבחרו. מחליף את החתימה הקודמת (p_stage text) ב-p_stages
-- text[] - drop קודם כי שינוי סוג פרמטר לא תואם ל-create or replace.
