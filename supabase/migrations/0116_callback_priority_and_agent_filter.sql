-- באג אמיתי, דווח בשטח: מי שביקש "חזרו אליי מאוחר יותר" לא קופץ שוב
-- לנציגים. הסיבה: 0114 מיין לפי "מעולם לא נוסה קודם" ראשון - אבל
-- callback_at<=now() כבר עבר סינון ב-WHERE (מותר לתפוס), רק שהוא תמיד
-- מפסיד לאלפי אנשי-קשר-שמעולם-לא-נוסו ב-ORDER BY, ונשאר תקוע לנצח מאחור.
-- callback הוא הבטחה עם זמן ספציפי - צריך לקבל עדיפות עליונה (אחרי
-- "דילוג אישי" בלבד), לא להתחרות עם "מי הכי ותיק בלי היסטוריה".
...
-- מוסיף סינון-אופציונלי לפי נציג ל-get_pending_callbacks (0099) - כדי
-- שטלפן יוכל לראות "רק מה שאני אישית קבעתי", לא את כל שיחות-החזרה
-- הממתינות של כל הצוות (זה נשאר גם ללוח-הניהול, p_agent_id null = הכל).
drop function if exists public.get_pending_callbacks(uuid);

create or replace function public.get_pending_callbacks(p_campaign_id uuid, p_agent_id uuid default null)
returns table(row_id uuid, contact_id uuid, first text, last text, phone text, callback_at timestamptz, scheduled_by uuid)
